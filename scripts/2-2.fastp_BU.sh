...
do
    ...
    if [ ${file_count} == 1 ]; then
        ...
        json=${output_dir}/${file}.json
        html=${output_dir}/${file}.html
        fastp --in1 ${fastq_1} --in2 ${fastq_2} --out1 ${out1} --out2 ${out2} --unpaired1 ${unpaired} --unpaired2 ${unpaired} -j ${json} -h ${html} --detect_adapter_for_pe --correction --trim_poly_g --thread 16 --length_required 30 --trim_front1 9 --trim_front2 9
    fi
done
conda deactivate